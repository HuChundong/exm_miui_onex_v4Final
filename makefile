...
# All apks from MIUI
local-miui-removed-apps  := LatinIME MediaProvider PackageInstaller GuardProvider LBESEC_MIUI Updater
...
local-phone-apps := AppSharing BackupRestoreConfirmation \
	...
	FilePicker FaceLock FM_Radio FMRadioService GoogleTTS HtcBeatsNotify \
	HTMLViewer LMW MarketUpdater \
	Nfc OneTimeInitializer SharedStorageBackup \
	Usage WifiRouter MediaProvider HTCAlbum PackageInstaller \
...
local-zip-misc:
	...
	@echo delete miui preinstall apps
	rm -f $(ZIP_DIR)/data/app/InputMethod.apk
	rm -f $(ZIP_DIR)/data/app/Userbook.apk
	rm -f $(ZIP_DIR)/data/app/MiuiVideo.apk
